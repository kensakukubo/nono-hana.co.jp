<?php
/**
 * 室内の様子（1枚表示・前後ボタン／ドット）
 *
 * @var int $post_id 対象固定ページID
 */
$post_id = isset( $post_id ) ? (int) $post_id : get_the_ID();
$slides  = function_exists( 'grouphome_get_room_gallery_slides' ) ? grouphome_get_room_gallery_slides( $post_id ) : [];
if ( empty( $slides ) ) {
	return;
}
$n        = count( $slides );
$slider_id = 'roomSlider-' . $post_id;
?>
<section class="guide-section facility-room-section">
	<div class="section-heading">
		<h2>室内の様子</h2>
		<p class="section-heading__sub">ROOM</p>
	...
	<div class="facility-room-slider" id="<?php echo esc_attr( $slider_id ); ?>" data-room-slider>
		<div class="facility-room-slider__viewport">
			...
			<figure class="facility-room-slider__slide<?php echo 0 === $i ? ' is-active' : ''; ?>" data-room-slide data-index="<?php echo (int) $i; ?>">
				<?php
				$img_alt = $slide['alt'] !== '' ? $slide['alt'] : get_the_title( $post_id );
				if ( ! empty( $slide['id'] ) ) {
